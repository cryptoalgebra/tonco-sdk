...
export function poolFactoryContractConfigToCell(
  ...
  return beginCell()
    ...
    .storeCoins(config.tonPrice ?? 0n)

    .storeRef(config.orderCode)

    .storeRef(config.nftContent)
    .storeRef(config.nftItemContent)
...
export function poolFactoryContractCellToConfig(
  ...
  config.nftContent = ds.loadRef();
  config.nftItemContent = ds.loadRef();

  console.log(`Remains b=${ds.remainingBits} r=${ds.remainingRefs}`);
  config.whitelist = ds.loadDict(
    Dictionary.Keys.BigUint(WHITELIST_KEY_LENGTH),
    WhitelistInfoEntry
  );

  //config.whitelist        = ds.loadDictDirect(Dictionary.Keys.BigUint(WHITELIST_KEY_LENGTH), WhitelistInfoEntry)
...
*/
  ...
  static factoryAddToWhitelistMessage(
    ...
    let body: Cell = beginCell()
      .storeUint(this.POOL_FACTORY_ADD_TO_WHITELIST, 32) // OP code
      .storeUint(0, 64) // query_id
      .storeAddress(address)
  ...
  static unpackFactoryAddToWhitelistMessage(
    ...
    let s = body.beginParse();
    const op = s.loadUint(32);
    if (op != this.POOL_FACTORY_ADD_TO_WHITELIST) {
      throw Error('Wrong opcode');
    }
    const query_id = s.loadUint(64);
  ...
  async sendFactoryAddToWhitelist(
    ...
    value: bigint,
    address: Address,
    allow_set_controller: bigint = 1n,
    allow_set_creator: bigint = 1n,
    allow_set_active: bigint = 1n,
    allow_set_settings: bigint = 1n,
    allow_set_content: bigint = 1n
  ) {
    await provider.internal(via, {
      value,
      sendMode: SendMode.PAY_GAS_SEPARATELY,
  ...
  static factoryDelFromWhitelistMessage(address: Address): Cell {
    let body: Cell = beginCell()
      .storeUint(this.POOL_FACTORY_DEL_FROM_WHITELIST, 32) // OP code
      .storeUint(0, 64) // query_id
      .storeAddress(address)
      .endCell();
    return body;
  ...
  static unpackFactoryDelFromWhitelistMessage(
    ...
    let s = body.beginParse();
    const op = s.loadUint(32);
    if (op != this.POOL_FACTORY_DEL_FROM_WHITELIST) {
      throw Error('Wrong opcode');
    }
    const query_id = s.loadUint(64);
    const address = s.loadAddress();
