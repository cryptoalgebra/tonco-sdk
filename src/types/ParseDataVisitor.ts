...
export class ParseDataVisitor implements StructureVisitor {
  ...
  visitField(field: MetaMessageField): void {
    if (this.skipFields) return;

    let workSlice = this.currentSlice();
    if (workSlice == null) return;
    ...
    if (field.type == 'Address') {
      ...
      if (address != null) value = address.toString();
      else value = 'addr_none';
    }
    if (field.type == 'Coins') {
    ...
    if (field.type == 'Cell') {
      if (field.meta.includes('Maybe')) {
        ...
        if (cell == null) value = '';
        else value = cell.toBoc().toString('hex');
      } else {
        value = workSlice
          .loadRef()
          .toBoc()
